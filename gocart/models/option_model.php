...
Class Option_model extends CI_Model
{
	...
	********************************************************************/
	...
	function update_option_values($value){
		if(isset($option['id'])){
			$this->db->where('id', $value['id']);
			$this->db->update('options_values', $value);	
		}
	}
	...
	function save_option($option, $values)
	{
		...
		foreach($values as $value)
		{
			...
			if(isset($value['id'])){
				$this->db->where('id', $value['id']);
				$this->db->update('option_values', $value);
			}else{
	...
	********************************************************************/
	...
	{
		return $this->db->get_where('option_values', array('id'=>$value_id))->row();
	}
	
	function save_option_quantity($option)
	{
		if(isset($option['id']))
		{
			$this->db->where('id', $option['id']);
			$this->db->update('option_values', $option);
			$id	= $option['id'];
		}
		return $id;
	}
