...
Class Customer_model extends CI_Model
{
	...
	********************************************************************/
	...
	function get_customer_group($id)
	{
		...
		if($value['group_id']!=0) 
		{
			$group = $this->get_group($value['group_id']);
			if($group) // group might not exist
			{
				if($group->discount_type == "fixed")
				{
					$value['group_discount_formula'] = "- ". $group->discount; 
				}
				else
				{
					$percent	= (100-(float)$group->discount)/100;
					$value['group_discount_formula'] = '* ('.$percent.')';
				}
			}
		}
	...
	{
		$address= $this->db->get('customers_address_bank')->result_array();
		return $address;
	}
	...
	function save($customer)
	{
		...
		{
			$this->insert_newsletter($customer['email']);
	...
	function insert_newsletter($email)
	{
		...
		$insert_query = $this->db->insert_string('newsletter', array('email'=>$email));
	...
	*/
				...
				if($group) // group might not exist
				{
					...
					else
					{
						...
						$customer['group_discount_formula'] = '* ('.$percent.')';
                                        ...
                                        $customer['group_discount_percentage'] = $group->discount;
				}
			}
	...
	function reset_password($email)
	{
		...
		if ($customer)
		{
			...
			$row['content'] = str_replace('{new_password}', $new_password, $row['content']);
			
			$this->email->from("service@scarletcollection.com", $this->config->item('site_name'));
			$this->email->to($email);
	...
	{
		return $this->db->where('id', $id)->update('customers', array('group_id'=>$group));	
	}
