...
Class order_model extends CI_Model
{
	...
	{			
        ...
        function get_orders_sales($search=false, $sort_by='', $sort_order='DESC', $limit=0, $offset=0)
	{			
		if ($search)
		{
			if(!empty($search->term))
		...
		$this->db->or_where_in('status', array('Processing', 'Shipped'));
		if($limit>0)
		{
			$this->db->limit($limit, $offset);
		}
	...
	{
		$this->db->where('id', $id);
		$this->db->update('orders', array('reminder'=>1));
	}
	...
	function get_customer_orders_pending()
	{
		//$this->db->or_where_in('status', array('Processing', 'Pending'));
		$this->db->where('status', 'Pending');
		return $this->db->get('orders')->result();
	}
	
	function get_percustomer_orders_pending($id)
	{
		//$this->db->or_where_in('status', array('Processing', 'Pending'));
		return $this->db->get_where('orders', array('customer_id'=>$id, 'status'=>'Pending'))->result();
	}
	
	function get_customer_orders_processing($id, $offset=0)
	{
               
		return $this->db->get_where('orders', array('customer_id'=>$id, 'status'=>'Processing'))->result();
	}
	
	function get_customer_orders_list($id, $offset=0)
	{
                 $this->db->order_by('ordered_on', 'DESC');
		return $this->db->get_where('orders', array('customer_id'=>$id), 8, $offset)->result();
	}
	
	function get_customer_orders_total_shop($id)
	{
		$this->db->join('order_items', 'orders.id = order_items.order_id');
		$this->db->order_by('ordered_on', 'DESC');
		
		$orders = $this->db->get_where('orders', array('customer_id'=>$id, 'orders.status'=>'Shipped'))->result();
		$total = 0;
       ...
       function get_order_total_shop($order_number){
	...
	function cancel_order($id)
	{
		...
		//now delete the order items
		$this->db->where('order_id', $id);
		$this->db->update('order_items', array('status'=>'Cancelled'));
	}
	...
	function save_order($data, $contents = false)
	{
		...
		if($contents)
		{
			...
			foreach($contents as $item)
			{
				...
				if($array['post_options'] != NULL){
					foreach ($array['post_options'] as $option_value) {
					  $option_value_id = $option_value;
					  //echo $option_value_id;
					}
					$save['option_value_id']	= $option_value_id;
				}
	...
	function get_best_sellers($start, $end)
	{
		...
		foreach($orders as $order)
		{
			...
			$order_items	= $this->db->select('product_id, quantity, option_value_id')->where(array('order_id'=>$order->id, 'status'=>'CONFIRM'))->get('order_items')->result_array();
			...
			foreach($order_items as $i)
			{
				...
				//{
				if(isset($items[$i['product_id'].".".$i['option_value_id']]))
				{
					$items[$i['product_id'].".".$i['option_value_id']]	+= $i['quantity'];
				}
				else
				{
					$items[$i['product_id'].".".$i['option_value_id']]	= $i['quantity'];
		...
		foreach($items as $key=>$quantity)
		{
			...
			$pieces = explode(".", $key);
			$id = $pieces[0];
			$option_value_id = $pieces[1];
			
			//echo $key;
			$product				= $this->db->where('id', $id)->get('products')->row();
			$variable['name'] = $product->name;
			
			//foreach($quantity as $key1=>$quantity1)
			//{
				if($option_value_id != 0){
					$option				= $this->db->where('id', $option_value_id)->get('option_values')->row();
					$variable['option'] = $option->name;
	...
	function insert_shipping_number($number=false, $id){
		//update the order with this order id
		$this->db->where('id', $id);
		if($number)
			$this->db->update('orders', array('shipping_number'=>$number, 'status'=>'Shipped', 'ordered_on'=>date('Y-m-d H:i:s')));	
		else
			$this->db->update('orders', array('status'=>'Shipped', 'ordered_on'=>date('Y-m-d H:i:s')));	
	}
