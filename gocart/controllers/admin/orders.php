...
class Orders extends Admin_Controller {	
	...
	function index($sort_by='order_number',$sortorder='desc', $code=0, $page=0, $rows=15)
	{
		...
		$config['base_url'] = base_url().'/'.$this->config->item('admin_folder').'/orders/index/'.$sort_by.'/'.$sortorder.'/'.$code.'/';
	...
	function packing_slip($order_id)
	{
		...
		//$html = $this->load->view($this->config->item('admin_folder').'/packing_slip.php', $data, true);
		// Get output html
		//$html = $this->output->get_output();
		// Convert to PDF
		//$this->dompdf->load_html($html);
		//$this->dompdf->render();
		//write_file('name.pdf', $this->dompdf->output());
	...
	function insert_number($order_id=false){
		$this->load->model('Customer_model');
		if($order_id){
			...
			{
				// get customer data
				$customer = $this->Customer_model->get_customer($data['order']->customer_id);
				$this->load->model('Messages_model');
				// get the list of canned messages (order)
    			        $msg = $this->Messages_model->get_message(3);	
				...
				// {shipping_method}
				$msg['content'] = str_replace('{shipping_method}', $data['order']->shipping_method, $msg['content']);
				
				// {shipping_number}
				$msg['content'] = str_replace('{shipping_number}', $this->input->post('shipping_number'), $msg['content']);
				$msg['content'] = str_replace('{customer_name}', $customer->firstname.' '.$customer->lastname, $msg['content']);
				...
				$this->email->message($msg['content']);
				...
				$this->Order_model->insert_shipping_number($this->input->post('shipping_number'), $order_id);				
				
				$this->load->view($this->config->item('admin_folder').'/iframe/insert_shipping_number.php', array('finished'=>1));
			
				return;	
			}else{
				$data['order_id'] = $order_id;
				$this->load->view($this->config->item('admin_folder').'/iframe/insert_shipping_number.php', $data);
				
			}
		}	
	}
    
	function send_shipped($order_id=false){
		if($order_id){
			// get the order details
			$data['order'] = $this->Order_model->get_order($order_id);
			...
			// fix html
			$msg['content'] = html_entity_decode($msg['content']);
			...
			// {shipping_number}
			$msg['content'] = str_replace('Your JNE shipping number: {shipping_number}', $this->input->post('shipping_number'), $msg['content']);
			$msg['content'] = str_replace('{customer_name}', $customer->firstname.' '.$customer->lastname, $msg['content']);
			...
			$this->Order_model->insert_shipping_number(false, $order_id);
    ...
    function send_notification($order_id='')
    {
    	...
    	foreach($data['msg_templates'] as &$msg)
    	{
			...
			// {shipping_number}
			$msg['content'] = str_replace('{shipping_number}', $data['order']->shipping_number, $msg['content']);
