...
class Cart extends CI_Controller {
	...
	function collection($sort_by="created", $sort_order="desc", $page=0)
	{
		...
		//set up pagination
		$this->load->library('pagination');
		$config['base_url']		= base_url().'collections/'.$sort_by.'/'.$sort_order.'/';
		$data['current_url'] = $config['base_url'];
		$config['uri_segment']	= 4;
		$config['per_page']		= 9;
		//$config['total_rows']	= $this->Product_model->count_category_products($data['category']->id);
		...
		if($page && $page == "all" ){
			...
			$data['products']	= $this->Product_model->get_products(false, false, false, $sort_by, $sort_order);
			$config['total_rows']	= $this->Product_model->count_products(false);
		}else{
		
			$data['products']	= $this->Product_model->get_products(false, $config['per_page'], $page, $sort_by, $sort_order);
			$config['total_rows']	= $this->Product_model->count_products(false);
		}
	...
	function add_to_cart()
	{
		...
		if( ! $status['validated'])
		{
			...
			$this->session->set_flashdata('message_product', '"'.$stock->name.'" has been added to cart. View Cart <a href="/cart/view_cart">here<a/>');
			redirect($this->Product_model->get_slug($product_id));
		}
