...
class Coupons extends Admin_Controller {	
  ...
  function create_return_guid($namespace = '') {     
		$guid = '';
		$uid = uniqid("", true);
		$data = $namespace;
		$data .= $_SERVER['REQUEST_TIME'];
  ...
  function create_coupon(){
		...
		if ( $this->input->get('quantity') ){
			for($i=1;$i<=$this->input->get('quantity');$i++){
				
				//default values are empty if the product is new
				$save['id'] = false;
				$save['code']					= $this->create_return_guid();
				//echo $save['code']."<br/>";
				$save['start_date']				= '2013-01-01';
				$save['whole_order_coupon']		= 1;
				$save['max_product_instances'] 	= '';
				$save['end_date']				= '2050-06-01';
				$save['max_uses']				= 1;
				$save['reduction_target'] 		= 'price';
				$save['reduction_type']			= 'fixed';
				$save['reduction_amount']		= '100000';
				
				// save coupon
				if(!$this->Coupon_model->check_code($save['code'], false)){
					$promo_id = $this->Coupon_model->save($save);
					echo $save['code'].'<br/>';
				}
			}
