...
class Checkout extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('Page_model', 'Settings_model', 'Location_model', 'Place_model', 'Order_model'));
	...
	function index()
	{
		...
		$this->session->set_flashdata('redirect', 'checkout');
		...
		$data["jne"] = $this->Place_model->get_jne_one_single($customer["ship_address"]["province_id"]);
		if($this->go_cart->shipping_method()==""){
			$this->go_cart->set_shipping_method('JNE REG');
			$this->go_cart->set_shipping_price($data["jne"]->reg*ceil($this->go_cart->order_weight()));
		}
	...
	function customer_details()
	{
		...
		$data['jne'] = $this->Place_model->get_jne_one_single($data['customer']["ship_address"]["province_id"]);
		
		$this->load->view('checkout/customer_details_static', $data);
	}
	...
	function save_customer()
	{
		...
		$this->form_validation->set_rules('ship_province_id', 'Shipping Province', 'trim|required|numeric');
		//$this->form_validation->set_rules('ship_city_id', 'Shipping State', 'trim|required|numeric');
		...
		if ($this->form_validation->run())
		{
			...
			//$ship_province = $this->Place_model->get_province($this->input->post('ship_province_id'));
			$ship_province = $this->Place_model->get_jne_one_single($this->input->post('ship_province_id'));
			
			//$ship_city = $this->Place_model->get_city($this->input->post('ship_city_id'));			
			...
			//$customer['ship_address']['city_id']		= $this->input->post('ship_city_id');  // use the id's to populate address forms
			$customer['ship_address']['province_id']		= $this->input->post('ship_province_id');
			//$customer['ship_address']['city']			= $ship_city->name;  // save the state for output formatted addresses
			//$customer['ship_address']['province']		= $ship_province->name; // some shipping libraries require country name
			$customer['ship_address']['province']		= $ship_province->place; // some shipping libraries require country name
			$customer['ship_address']['country']		= $this->input->post('country');
			
			$customer['bill_address']['firstname']		= $this->input->post('ship_firstname');
			$customer['bill_address']['lastname']		= $this->input->post('ship_lastname');
			$customer['bill_address']['email']			= $this->input->post('ship_email');
			$customer['bill_address']['phone']			= $this->input->post('ship_phone');
			//$customer['bill_address']['company']		= $this->input->post('ship_company');
			$customer['bill_address']['address1']		= $this->input->post('ship_address1');
			$customer['bill_address']['address2']		= $this->input->post('ship_address2');
			//$customer['bill_address']['city']			= $this->input->post('ship_city');
			$customer['bill_address']['zip']			= $this->input->post('ship_zip');
			//$customer['bill_address']['city_id']		= $this->input->post('ship_city_id');  // use the id's to populate address forms
			$customer['bill_address']['province_id']		= $this->input->post('ship_province_id');
			//$customer['bill_address']['city']			= $ship_city->name;  // save the state for output formatted addresses
			$customer['bill_address']['province']		= $ship_province->place; // some shipping libraries require country name
			...
			//$shipping_cost = $this->Place_model->get_cost($customer['ship_address']['province_id'], $customer['ship_address']['city_id']);
			// Free shipping coupon applied ?
			if(!$this->go_cart->is_free_shipping()) 
			{
				// add free shipping as an option, but leave other options in case they want to upgrade
				$this->go_cart->set_shipping_price($ship_province->reg*ceil($this->go_cart->order_weight()));
				
			}else{
				$this->go_cart->set_shipping_price(0);
			}
			
			/*if($customer['ship_address']['country']!="Indonesia"){
				$this->go_cart->set_shipping_method('Post Indonesia');
			}else{
				$this->go_cart->set_shipping_method('JNE REG');
			}*/
			$this->go_cart->set_shipping_method('JNE REG');
			...
			//customer details
			$return['view'] = $this->load->view('checkout/customer_details_static', array('customer'=>$customer,'address_update'=>1,'jne'=>$this->Place_model->get_jne_one_single($customer["ship_address"]["province_id"])), true);
	...
	function save_shipping_method()
	{
		...
		{	
			...
			$this->go_cart->set_shipping( $shipping[0], 0);
		}
	}
	
	function save_shipping_method_price()
	{
		$method = $this->input->post('method');
		$price = $this->input->post('price');
		$this->go_cart->set_shipping_method($method);
		$this->go_cart->set_shipping_price($price);
	}
	
	function order_summary()
	{
		//to redirect back to this page after successfully login
		//$this->session->set_flashdata('redirect', 'checkout/order_summary');
		$this->session->set_flashdata('redirect', 'checkout/order_summary');
		//make sure they're logged in if the config file requires it
		$customer = $this->go_cart->customer();
		if (!isset($customer['id']))
		{
			...
			$this->load->view('checkout/summary_test', $data);
		}else{
		$data['login_check'] = TRUE;
		$data['customer']	= $this->go_cart->customer();
		$data['check_address'] = TRUE;
		
		if(!isset($data['customer']['ship_address'])){
			...
			$data['customer']	= $this->go_cart->customer();
			$customerGroup = $this->Customer_model->get_customer_group($data['customer']['id']);
			$this->go_cart->set_customer_group($customerGroup);
			
			//$shipping_cost = $this->Place_model->get_cost($data['customer']['ship_address']['province_id'], $data['customer']['ship_address']['city_id']);
			$shipping_cost = $this->Place_model->get_jne_one_single($data['customer']['ship_address']['province_id']);
			
			if(!$this->go_cart->is_free_shipping()) 
			{
				// add free shipping as an option, but leave other options in case they want to upgrade
				if($this->go_cart->shipping_method()=="JNE REG"){
					$this->go_cart->set_shipping_method('JNE REG');
					$this->go_cart->set_shipping_price($shipping_cost->reg*ceil($this->go_cart->order_weight()));
				}else{
					$this->go_cart->set_shipping_method('JNE YES');
					$this->go_cart->set_shipping_price($shipping_cost->yes*ceil($this->go_cart->order_weight()));
				}
				
				
				/*if($data['customer']['ship_address']['country']!="Indonesia" || !isset($data['customer']['ship_address']['country'])){
					$this->go_cart->set_shipping_method('Post Indonesia');
				}else{
					$this->go_cart->set_shipping_method('JNE REG');
				}*/
	...
	{
		$module = $this->input->post('payment_method');
		//$module = 'paypal_express';
		
		if($module=='bank_transfer')
		{
			$this->go_cart->set_payment('Transfer', 'Transfer' );
	...
	{		
		...
		if($this->config->item('require_login'))
		{
			$this->Customer_model->is_logged_in(false, "https://www.scarletcollection.com/secure/login");
		}
		...
		$data['shipping']			= 'JNE REG';//$this->go_cart->shipping_method();
		$data['payment']			= 'Transfer';//$this->go_cart->payment_method();
		$data['customer']			= $this->go_cart->customer();
		...
		$html = $this->load->view('packing_slip', $data, true);
		// Get output html
		//$html = $this->output->get_output();
		// Convert to PDF
		$this->dompdf->load_html($html);
		$this->dompdf->render();
		write_file('invoice/'.$order_id.'.pdf', $this->dompdf->output());
		...
		$row['content'] = str_replace('{order_number}', $data['order_id'], $row['content']);
		...
		$this->email->attach('./invoice/'.$order_id.'.pdf');
	...
	{		
		...
		if($this->config->item('require_login'))
		{
			$this->Customer_model->is_logged_in(false, "http://dev.scarletcollection.com/secure/login");
		}
		...
		if($payment['module']=='paypal_express'){
			...
			//When paypal payment is confirmed
			if($this->go_cart->total() > 0 && isset($payment['confirmed'])) {
				$this->load->model('Payment_transfer_model');
			
				$paypal_data['payer_id'] = $payment['details']['PAYERID'];
				$paypal_data['express_checkout_token'] = $payment['details']['TOKEN'];
				$paypal_data['version'] = $payment['details']['VERSION'];
				$paypal_data['transaction_id'] = $payment['details']['TRANSACTIONID'];
				$paypal_data['gross_amount'] = $payment['details']['AMT'];
				$paypal_data['fee_amount'] = $payment['details']['FEEAMT'];
				$paypal_data['net_amount'] = $payment['details']['AMT'] - $payment['details']['FEEAMT'];
				$paypal_data['status'] = $payment['details']['PAYMENTSTATUS'];
				
				$this->Payment_transfer_model->save_payment_paypal($paypal_data);
				
				$data['payment_transaction_id'] = $payment['details']['TRANSACTIONID'];
				$data['payment']['total'] = $payment['details']['AMT'];
			}
		}
		
		
		//// save the order
		$order_id = $this->go_cart->save_order($data['payment_transaction_id']);
		
		$data['order_id']			= $order_id;
		$data['shipping']			= $this->go_cart->shipping_method();
		if(isset($payment['module']) && $payment['module']=='paypal_express'){
			$data['payment']['details']			= 'Paypal Express';//$this->go_cart->payment_method();
		}else{
			$data['payment']['details']			= 'Transfer';//$this->go_cart->payment_method();
		}
		$data['customer']			= $this->go_cart->customer();
		$data['additional_details']	= $this->go_cart->additional_details();
		...
		$this->email->from('sales@watchinc.co.id', $this->config->item('company_name'));
		...
		//To clear email variables
		$this->email->clear(TRUE);
		
		if(isset($payment['module']) && $payment['module']=='paypal_express'){
			if(isset($payment['confirmed'])){
				$row_paypal = $this->messages_model->get_message(15);
				
				$row_paypal['content'] = html_entity_decode($row_paypal['content']);
			
				// set replacement values for subject & body
				// {customer_name}
				$row_paypal['subject'] = str_replace('{customer_name}', $data['customer']['firstname'].' '.$data['customer']['lastname'], $row_paypal['subject']);
				$row_paypal['content'] = str_replace('{customer_name}', $data['customer']['firstname'].' '.$data['customer']['lastname'], $row_paypal['content']);
				
				// {order_number}
				$row_paypal['subject'] = str_replace('{order_number}', $data['order_id'], $row_paypal['subject']);
				
				// {url}
				$row_paypal['subject'] = str_replace('{url}', $this->config->item('base_url'), $row_paypal['subject']);
				$row_paypal['content'] = str_replace('{url}', $this->config->item('base_url'), $row_paypal['content']);
				
				//$row_paypal['content'] = str_replace('{order_number}', $data['order_id'], $row['content']);
				
				// {site_name}
				$row_paypal['subject'] = str_replace('{site_name}', $this->config->item('company_name'), $row_paypal['subject']);
				$row_paypal['content'] = str_replace('{site_name}', $this->config->item('company_name'), $row_paypal['content']);
				...
				$this->email->subject($row_paypal['subject']);
				$this->email->message($row_paypal['content']);
