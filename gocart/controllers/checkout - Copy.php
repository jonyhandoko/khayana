...
class Checkout extends CI_Controller {
	...
	function save_customer()
	{
		...
		$this->form_validation->set_rules('ship_province_id', 'Shipping Country', 'trim|required|numeric');
		...
		if ($this->form_validation->run())
		{
			...
			$customer['bill_address']['province']		= $ship_province->name; // some shipping libraries require country name
	...
	function order_summary()
	{
		...
		$this->go_cart->set_shipping_price($this->input->post('shipping_cost')*ceil($this->go_cart->order_weight()));
	...
	{		
		...
		$data['shipping']			= 'JNE';//$this->go_cart->shipping_method();
		...
		if(($this->Order_model->get_customer_orders_total_shop($data['customer']['id'])+$this->go_cart->total()) > 1000000){
			$this->Customer_model->set_group($data['customer']['id'],2);
		}
