...
function format_address($fields)
{
	...
	//$c_data = $CI->place_model->get_country($fields['country_id']);
...
function format_currency($value, $symbol=true)
{
	...
	// retrieve settings
	if ( $settings = $CI->Settings_model->get_settings('paypal_express') ) 
	{		
	...
	if(isset($_SESSION['currency']) && $_SESSION['currency'] == 'USD'){
		$formatted = number_format(abs($value/$convert), 2	, '.', '.');
		$formatted = $neg.'US $'.$formatted;
	}else{
		$formatted = number_format(abs($value), 0, '.', '.');
		$formatted = $neg.$CI->config->item('currency_symbol').$formatted;
	}
