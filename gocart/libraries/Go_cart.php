...
class go_cart {
	...
	function __construct() 
	{
		...
		$this->CI->load->model(array('Coupon_model' , 'Gift_card_model', 'Customer_model',  'Settings_model', 'Digital_Product_model'));
		...
		$paypal_settings = $this->CI->Settings_model->get_settings('paypal_express');
		$this->convert = $paypal_settings['convert'];
	...
	private function _init_properties($totals_only=false, $preserve_customer=false)
	{
		...
		$this->_cart_contents['group_discount_percentage']		= 0;
		...
		$this->_cart_contents['gift_card_discount_percentage'] 		= 0;
		...
		if( ! $totals_only) 
		{
			...
			//$this->_cart_contents['shipping']['prices']	= 0.00;
			$this->_cart_contents['shipping2']['prices']	= 0.00;
			$this->_cart_contents['shipping_price']	= 0.00;
...
//			$str = ' - ($subtotal * ('. $reduction_ammount .' /100))';
		...
		if($recalculate) 
		{
			...
			foreach ($this->_cart_contents['items'] as $key => &$val)
			{
				...
				if(isset($this->_cart_contents['customer']['group_discount_formula']))
				{
					...
					$this->_cart_contents['group_discount_percentage'] = $this->_cart_contents['customer']['group_discount_percentage'];
				} else {
			...
			if($this->_cart_contents['requires_shipping']) 
			{
				if(!$this->is_free_shipping()){
					$this->_cart_contents['cart_total'] += $this->_cart_contents['shipping2']['prices'];
				}else{
					$this->_cart_contents['shipping2']['prices']  = 0.00;
				}
			} else {
	...
	********************************************************/
	...
	// set shipping price
	function set_shipping_price($price)
	{
		if(!is_numeric($price)) return false;
		
		$this->_cart_contents['shipping2']['prices']=(float)$price;
		$this->_cart_contents['wow']=(float)$price;
		//update cart - recalculate
		$this->_save_cart();
	}
	function get_wow(){
		return $this->_cart_contents['wow'];
	}
	// set shipping method
	...
	// set customer group
	function set_customer_group($value)
	{
		//if(!is_numeric($groupId)) return false;
		
		$this->_cart_contents['customer']['group_id']=$value['group_id'];
		$this->_cart_contents['customer']['group_discount_formula']=$value['group_discount_formula'];
	...
	//To save return value from paypal
	function set_payment_paypal($final){
		$this->_cart_contents['payment']['details'] = $final;
		// save cart - no recalculation necessary
		$this->_save_cart(false);
	}
	
	// This saves the confirmed order 
	function save_order($payment_transaction_id) {
		...
		//$save['ship_city_id']		= $ship['city_id'];
		$save['ship_province']		= $ship['province'];
		$save['ship_province_id']	= $ship['province_id'];
		/*$save['ship_city']			= $ship['city'];
		$save['ship_zip']			= $ship['zip'];
		...
		//$save['bill_city_id']		= $bill['city_id'];
		$save['bill_province']		= $bill['province'];
		$save['bill_province_id']	= $bill['province_id'];
		/*$save['bill_city']			= $bill['city'];
		$save['bill_zip']			= $bill['zip'];
		...
		$save['shipping_method']	= $this->_cart_contents['shipping'];
		$save['shipping']			= $this->_cart_contents['shipping2']['prices'];
		...
		$save['group_discount'] = $this->_cart_contents['group_discount'];
		$save['group_discount_percentage'] = $this->_cart_contents['group_discount_percentage'];
		...
		/*if(!empty($this->_cart_contents['payment']['description']))
		{
			...
			if($save['payment_info'] == "Paypal Express"){
		...
		if(!empty($payment_transaction_id)){
			$save['payment_transaction_id'] = $payment_transaction_id;
		}
		...
		foreach ($this->_cart_contents['items'] as $item)
		{
			...
			if($item['post_options'] != NULL){
				foreach ($item['post_options'] as $k => $v) {
				  $option_value_id = $v;
				}
				$this->CI->load->model('Option_model');
				
				$option			= $this->CI->Option_model->get_value($option_value_id);
				$option_new_quantity	= intval($option->qty) - intval($item['quantity']);
				$option_quantity	= array('id'=>$option->id, 'qty'=>$option_new_quantity);
				$this->CI->Option_model->save_option_quantity($option_quantity);
			}
		}
	...
	function conversion_total()
	{
		return round($this->_cart_contents['cart_total']/$this->convert, 2);
	}
	...
	function conversion_subtotal()
	{
		//return $this->_cart_contents['shipping2']['prices'];
		
		return round($this->_cart_contents['cart_subtotal']/$this->convert, 2);
	}
	function group_discount()
	{
		return $this->_cart_contents['group_discount'];
	}
	function conversion_group_discount()
	{
		//return $this->_cart_contents['shipping2']['prices'];
		
		return round($this->_cart_contents['group_discount']/$this->convert, 2);

	}
	function group_discount_percentage()
	{
		return $this->_cart_contents['group_discount_percentage'];
	}
	function coupon_discount()
	{
		return $this->_cart_contents['coupon_discount'];
	}
	function conversion_coupon_discount()
	{
		return round($this->_cart_contents['coupon_discount']/$this->convert, 2);
	}
	...
	function is_free_shipping()
	{
		return false;
		if( ! $this->_cart_contents['free_shipping_coupon'] ) {
			if($this->_cart_contents['cart_total']>500000)
				return true;
			else
				return false;
	...
	{
		if(isset($this->_cart_contents['shipping']))
	...
	function conversion_shipping_cost()
	{
		//return $this->_cart_contents['shipping2']['prices'];
		
		return round($this->_cart_contents['shipping2']['prices']/$this->convert, 2);
	}
