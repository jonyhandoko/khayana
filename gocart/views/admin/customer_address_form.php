...
<div id="gc_address" style="overflow:auto">
	...
	<div class="field_wrap">
     ...
     <div class="field_wrap">
		<div>
			...
			<?php echo form_dropdown('country', array("Indonesia"=>"Indonesia","Singapore"=>"Singapore"), set_value('country', $country), 'style="width:200px; display:block;" id="country_id" class="input"');?>
		</div>
	</div>
	...
	$(document).ready(function(){
		...
		$('#f_province_id').change(function(){
			$.post('<?php echo site_url($this->config->item('admin_folder').'/places/get_cities_menu');?>',{provinceId:$('#f_province_id').val()}, function(data) {
			  $('#f_city_id').html(data);
			});
